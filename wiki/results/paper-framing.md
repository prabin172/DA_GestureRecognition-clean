...
Abstract event-driven controller (no physics sim): what earns the THMS venue. Gesture subset → 7 System Inputs (2 Safety-Critical States, 5 Routine States — no physical-action names, per the 2026-07-16 terminology revision, see [[controller]]); posterior stream from real held-out clips through the actual recognizer; FSM + safety layer (temporal smoothing, confidence threshold with reject, dwell filter, error recovery, safe-home). Metrics: task success, time-to-completion, false-activation rate, corrective inputs, rejection rate, latency — across 4 inits × k; one confidence-threshold sweep → safety/throughput curve. Design requirements: sequential asymmetric task (errors compound), held-out non-target gestures as distractor stream, tune on validation not test, Safety-Critical States on reliably-recognized gestures ([[oov-leave-class-out]]). **Sequencing: build only after recognizer numbers are final.** Each step human-OK'd first. **Built and locked** — see [[phase3-controller]].

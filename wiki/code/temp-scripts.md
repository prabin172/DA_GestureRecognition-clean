...
| script | role |
|---|---|
| `temp_loso_fulltrain_calibration.py` | **THE main experiment script** — full-LOSO base training + k-shot head-only calibration → [[loso-fulltrain-calibrate]] |
...
| `temp_mmd_domain_gap.py` | MMD NTU↔Xsens per encoder → [[mmd-domain-gap]] |
| `temp_loso_leave_class_out_fewshot.py` | OOV protocol → [[oov-leave-class-out]] |
| `temp_pretrain_supcon_ntu.py` | SupCon NTU pretraining |
| `temp_ntu_to_ntu_objective_sanity.py`, `temp_xsens_only_loso_objective_sanity.py` | within-domain sanity → [[sanity-checks]] |
...
- Ablations: `temp_source_relatedness_ablation.py`, `temp_supmae_lambda_sweep.py`, `temp_loso_leave_class_out_fewshot.py`, `temp_loso_mae_supmae_jointft_calibration.py`

...
### B8. A2 multi-seed pooling — reproducibility gap closed, 2026-07-13

New `scripts/main_experiment/analyze_a2_multiseed.py` reproduces the pooled A2 table (wiki `a2-subject-scaling.md`, previously traced to an uncommitted "ad hoc" script in the sibling `DA_GestureRecognition` repo) directly from `trained_models/A2-subjectScaling{,-seed43,-seed44}/N{0..4}/summary.csv`. Verified byte-identical against the existing `a2_pooled_results.csv` (diff, sorted, empty). Paired-t companion (`a2_pooled_stats.csv`, cells keyed on (subject,seed), n≤15) reproduces every p-value already cited in the wiki page exactly: supLP120 N=1,k=1 mean_diff=+18.43 p=7.90e-06; N=1,k=3 mean_diff=+27.46 p=2.07e-06; mae N=4,k=1 mean_diff=-2.84 p=1.80e-03 (wiki cites p=.0018 — matches to reported precision). No numbers changed by this entry — pure reproducibility fix, script now committed here (this repo is now the sole active working copy; the sibling `DA_GestureRecognition` repo's local copy has been retired).

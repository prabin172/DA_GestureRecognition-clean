#!/usr/bin/env python3
"""CZU-MHAD published-baseline reproduction: statistical-moment features + CRC.

Reproduces the CZU-MHAD dataset paper's recognizer family (Chao et al., IEEE
Sensors J. 2022): per-dimension statistical moments (mean/std/var/skew/kurtosis)
+ a Collaborative Representation Classifier (CRC-RLS, lambda=1e-4). Applied to
OUR shared LRQ representation on the EXACT SAME LOSO k-shot splits the learned
recognizer used (byte-identical calib/eval partitions read from the split JSONs)
-> a head-to-head "hand-crafted+CRC vs learned encoder" comparison on identical
splits, isolating the value of the learned/pretrained encoder.

Outputs -> trained_models/CZU-skeleton-LOSO/crc_baseline/
  crc_per_fold.csv     per (subject,k) accuracy + macro-F1
  crc_summary.csv      per-k mean over 5 folds
  comparison.csv       CRC baseline vs the learned recognizer (from summary.csv)
"""
import os, json, glob
import numpy as np
import pandas as pd
...
DATA = os.path.join(ROOT, "Data_Processed/czu_skeleton_lrq")
RUN = os.path.join(ROOT, "trained_models/CZU-skeleton-LOSO")
...
def label_of(fname):
    return fname.split("__")[1]                    # 'a01'..'a22' == class id
...
def main():
    ...
    for k in KS:
        for s in SUBJECTS:
            ...
            # z-score (train stats) then L2-normalize each sample (standard CRC preproc)
    ...
    print("\nCRC baseline (mean over 5 LOSO folds):")
    print(summ.to_string(index=False))

    # comparison vs the learned recognizer
    ...
    print("\nComparison — CRC baseline vs learned recognizer (mean acc over 5 folds):")

...
To complete the objective family named in the abstract, we add a fifth pretraining objective --- Khosla et al. (2020) SupCon, label-supervised contrastive learning (in-batch same-label positives) --- trained on NTU-120 identically to the other four, and re-run every analysis above (R1 linear probe and cross-domain LOSO-v2, R2 McNemar, R3 CKA, R4 ECE/AUC/convergence, R5 controller robustness, R6/R6b/R6c/R6d external datasets) with supcon included. Where an analysis depends on a fixed artifact shared across methods (the controller's illustrative command vocabulary, \S{}8), adding a fifth method can silently perturb that artifact if its construction pools over all loaded methods rather than a fixed subset --- we discovered and document one such instance (Results R5) rather than either hiding it or discarding the extension.

\subsection{8. Control-reliability simulation (C6 / THMS pillar)}

An abstract, event-driven controller consumes the \textbf{actual recognizer's posterior stream} --- no physics engine (the scientific content is uncertainty$\rightarrow$reliability propagation; a simulator would add risk without signal). \texttt{temp\_controller\_sim.py}.

\begin{itemize}
  \item \textbf{Command mapping.} The seven control primitives \{next, previous, approach, grasp, release, confirm, cancel\} are a hypothetical pick-and-place command vocabulary, not gestures present in the dataset --- none of the 22 recorded classes is itself a grasp/release action. Seven of the 22 real gestures are instead \textit{relabeled} as these primitives, purely by recognition reliability: gestures are ranked by mean recall pooled over \textit{all} methods at k=3, locomotion/whole-body-transition classes are excluded from command duty, and the most reliably-recognized remaining gestures are assigned to the most safety-critical primitives (grasp, release) --- e.g. in the illustrative run, \texttt{squat} (99.7\% recall) is relabeled \texttt{grasp} and \texttt{bow} (98.5\%) is relabeled \texttt{release}. The assignment is deliberately non-semantic (it tunes on aggregate reliability, favoring no single objective, rather than on which gesture "looks like" a command) --- the mission and its command vocabulary are a vehicle for studying how recognition and calibration propagate to task-level outcomes, not a claim that this dataset contains real grasp/release gestures.
  \item \textbf{Mission.} An ordered 12-step pick-place sequence containing two \texttt{grasp}s and one \texttt{release}, so recognition errors on safety-critical steps compound.
  \item \textbf{FSM + safety layer.} For each mission step with intended command c, a held-out clip of the mapped gesture is sampled and its recognizer posterior consumed. A \textbf{confidence-threshold reject} ($\tau$) re-prompts when max-softmax < $\tau$ (dwell/temporal-smoothing option requires agreement across repeated reads). A correct command advances the FSM; a **wrong safety-critical command --- issuing or being mistaken \textit{into} grasp/release --- is an immediate mission failure** (dangerous action); a recoverable wrong command incurs a cancel-and-reissue cost. Excessive consecutive rejections abort the mission.
  \item \textbf{Distractor / null stream.} Clips whose true class is \textit{not} a command gesture drive the \textbf{false-activation rate} (fraction that pass $\tau$ and map to a command primitive).
  \item \textbf{Metrics}, reported per (init $\times$ k) and swept over $\tau$: task success rate, mean time-to-completion (in cost units), rejection rate, corrective-commands-per-mission, and false-activation rate. Monte-Carlo (1000--3000 missions/condition), fixed RNG seed.
\end{itemize}

\subsubsection{8.1 Robustness protocol --- locking the design knobs}
A controller has design parameters (command vocabulary, error-cost model, operating threshold) that could each be tuned to manufacture a result. Instead of freezing arbitrary values we demonstrate the method \textbf{ordering is invariant} to all three, via three locks (\texttt{temp\_controller\_robust.py}; outputs \texttt{trained\_models/Phase3-controller/robust/}):

\begin{itemize}
  \item \textbf{Lock 1 --- randomized command vocabulary.} The 7-gesture$\rightarrow$primitive mapping is resampled at random 120 times; we report the \textit{distribution} of mission success across vocabularies and the fraction of vocabularies in which each pairwise method ordering holds. This removes any dependence on a hand-picked command set.
  \item \textbf{Lock 2 --- critical-cost sweep + two outcome models.} In a single pass we compute both a \textbf{hard-safety} outcome (any critical error $\rightarrow$ mission failure, binary success) and a \textbf{soft-cost} outcome (critical errors are recoverable but incur a penalty C\_crit), sweeping C\_crit $\in$ \{2, 5, 10, 20, 50, $\infty$\} ($\infty$ recovers hard-safety). This shows the ordering is independent of how catastrophic critical errors are made.
  \item \textbf{Lock 3 --- iso-safety operating point.} Rather than tune $\tau$ on the success metric, we fix a false-activation \textit{budget} (1\%, 0.5\%), find the smallest $\tau$ meeting it per method, and compare throughput there --- a deployment-standard, tuning-free way to set thresholds. The full $\tau$-frontier (task-success vs false-activation) is also reported.
\end{itemize}

The prototype single-configuration run (\texttt{temp\_controller\_sim.py}) is retained for illustration; all reported controller claims come from the robustness protocol. The base cost weights (T\_exec = 1, T\_reject = 1, T\_correct = 3, max consecutive rejects = 5) are held fixed while the load-bearing knobs are swept as above.

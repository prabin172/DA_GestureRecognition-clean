...
1. **Recognition differences compound into mission-level differences** (R5: a 4 pp recognition
   gap becomes a ~21 pp mission-success gap under the simulated FSM).
...
- **Per-subject session (~60–90 min):**
  ...
  2. Record k=3 calibration shots live, per gesture in the fixed 7-primitive vocabulary
  ...
  3. On-device calibration: head-only fine-tune from the frozen pretrained encoder, same
     ...
     between suit-up and the mission (seconds, not minutes; head-only fine-tune on 512-d
     pooled features is cheap).
  4. Perform the 12-step pick-place mission from `temp_controller_sim.py` (`MISSION = ["next",
     "next", "previous", "approach", "grasp", "confirm", "next", "approach", "grasp",
     "release", "confirm", "cancel"]`) against a **screen-based simulated executor** — no
...
- **Conditions, within-subject, counterbalanced:** 2 inits (`supLP120` = best-calibrated per R4a
  ...
  false-activation budget from R5 Lock 3) = **4 conditions, ~3 mission repetitions each**
  (~12 missions/subject, consistent with the ~60–90 min budget).
- **Metrics:** mission success (binary), time-to-completion, false-activation count, rejection
  count (under gating), corrective-command count; NASA-TLX questionnaire per condition
...
- **Analysis:** paired within-subject comparisons (each subject sees all 4 conditions); step-level
  McNemar across the ~36 mission repetitions per subject for power at small N (mirrors the
...
jump, stand`), and assign the most-reliable remaining gestures to the most safety-critical
primitives in order `grasp, release, confirm, approach, cancel, next, previous`. This is the same
design guard already stated in `RESEARCH_LOG.md` A0 (assign safety-critical commands to
reliably-recognized gestures — crossarms/squat/wave reliable, throw/jump/hop not, per the OOV
distinctiveness finding). **Fix the mapping once** (from the existing offline posteriors, before
any live session) so all subjects use the identical 7-gesture vocabulary — this keeps the live
study a direct extension of R5's Lock-1 vocabulary-robustness result rather than a new variable.

...
\subsubsection{R5b --- Does the controller's ranking hold beyond NTU$\rightarrow$Xsens?}

R5 above evaluates all three robustness locks on one transfer setting (NTU$\rightarrow$Xsens). We additionally reproduce \textbf{Lock 1 and Lock 2 only} (no iso-safety) on every other transfer setting with compatible per-clip posteriors --- CZU-MHAD skeleton (R6), CZU-MHAD IMU orientation-only (R6b), UTD-MHAD skeleton (R6d), CZU-MHAD dual-branch raw+quat (R6c) --- using the identical protocol: 120 independently-random 7-gesture System Input assignments drawn fresh from each setting's own label space, 1000 Monte Carlo trials per (assignment, method, k, $\tau$/C\_crit) cell. The CZU dual-branch checkpoints from R6c were not originally saved (only aggregate accuracy was); this check required retraining that setting's five priors $\times$ three seeds specifically to obtain per-clip posteriors, verified afterward to match the original R6c accuracy table within $\le$0.3 pp at every (method, k) cell --- same ranking, well inside the project's established training-non-determinism noise floor.

Worst/best method by Lock 1 (mean hard task-success, k=1, $\tau$=0) and Lock 2 (median mean-cost, k=1, C\_crit=50), across the 120 assignments per setting:

\begin{table*}[!t]
\centering
\caption{R5b --- Does the controller's ranking hold beyond NTU$\rightarrow$Xsens?}
\small
\begin{tabular}{llllll}
\toprule
setting & Lock1 worst & Lock1 best & Lock2 worst & Lock2 best & locks agree? \\
\midrule
NTU$\rightarrow$Xsens (R5 above) & mae (0.455) & supMAE (0.525) & mae (54.8) & supMAE (46.5) & \textbf{yes} \\
...
CZU dual-raw (R6c) & supcon (0.567) & scratch (0.619) & supcon (42.4) & scratch (39.0) & \textbf{yes} \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{The two locks agree with each other exactly on the two settings where the underlying recognition-level effect is itself large and significant.} On NTU$\rightarrow$Xsens, mae is the paper's established, seed-stable negative-transfer culprit (R2, McNemar p<.001 every k); both locks name it worst. On CZU dual-raw, supcon is the single worst pooled recognition-level result in the entire paper (R6c, sign p<.0001, paired-t p$\le$.0066 every k); both locks name it worst, and \texttt{dual/scratch} --- R6c's best performer at every k --- is both locks' best. \textbf{On the three settings where the underlying recognition-level effect is small or non-significant (CZU skeleton's mae is flat, not negative, R6; UTD's mae trends positive, not negative, R6d; CZU IMU orientation-only's supLP120-worst trend does not clear significance post-retrain, R6b), the two locks disagree with each other on which method is worst.} Best is more stable --- the two locks agree on best in 4/5 settings, and supcon or the strong-target scratch baseline wins in every setting except NTU$\rightarrow$Xsens (where supMAE narrowly leads, supcon a close second). CZU IMU orientation-only is a degenerate case beyond disagreement: recognition accuracy is low enough (55--60\%) that the 12-step, three-safety-critical task structure compresses every method's task success to a near-floor 10.7--12.4\%, at which point method-level differences are dominated by noise rather than signal.

\textbf{Reading:} the controller behaves as a compounding stress-test harness on top of the recognition table, not as an independent source of ranking. It sharpens a real, significant recognition-level effect into a large, lock-consistent task-level gap (NTU$\rightarrow$Xsens, CZU dual-raw); where the recognition-level effect is itself marginal, compounding amplifies noise into lock-to-lock disagreement rather than manufacturing a spurious consensus. This is a mechanistic echo of R6e's five-setting arc at the task-success/cost level, not an independent discovery --- the controller re-exposes the existing recognition-level pattern faithfully where that pattern is real, and unreliably where it is not.
...
\begin{itemize}
  ...
  \item R5b cross-setting (Locks 1--2 only): \texttt{trained\_models/Phase3-controller/crosssetting/\{ntu\_xsens,czu\_skeleton,czu\_imu\_quat,utd\_skeleton,czu\_dual\_raw\}/\{vocab\_sweep,vocab\_ordering,costmodel\_sweep,costmodel\_summary\}.csv} + \texttt{cross\_setting\_summary.csv} (\texttt{scripts/controller/controller\_crosssetting.py --vocabs 120 --missions 1000}). Posteriors: \texttt{dump\_posteriors.py} for the three settings with existing checkpoints (no retrain); CZU dual-raw posteriors required retraining \texttt{scripts/external/czu/dualbranch.py --mode dual} (3 seeds $\times$ 5 priors, \texttt{scripts/orchestration/10\_czu\_dual\_controller\_retrain.sh}) into new out-roots \texttt{trained\_models/CZU-IMU-DUAL-controller\{,-seed43,-seed44\}/} --- the original R6c \texttt{CZU-IMU-DUAL*/summary.csv} is untouched.
